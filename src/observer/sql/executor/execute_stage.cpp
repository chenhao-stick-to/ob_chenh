...
RC ExecuteStage::handle_request(SQLStageEvent *sql_event)
{
  ...
  if (physical_operator != nullptr) {
    return handle_request_with_physical_operator(sql_event);//产生tuple_schema
  }
  ...
  switch (stmt->type()) {
    case StmtType::SELECT: {
      ...
      for (const Field &field : select_stmt->query_fields()) {
        if (with_table_name) {
          schema.append_cell(field.table_name(), field.field_name());//将属性值名和对应表名加入schema
        } else {
          schema.append_cell(field.field_name());//直接将属性加入schema
        }
      }
    } break;
