...
{
  RC rc = write_result_internal(event, need_disconnect);//有执行计划，这里实际执行执行计划
  if (!need_disconnect) {
...
RC PlainCommunicator::write_result_internal(SessionEvent *event, bool &need_disconnect)
{
  ...
  rc = sql_result->open();//这里是执行计划的实际执行代码。根据schema来和record以及bufferpool等交互
  if (OB_FAIL(rc)) {
    sql_result->close();
  ...
  for (int i = 0; i < cell_num; i++) {
    const TupleCellSpec &spec = schema.cell_at(i);//表头信息的显示，在查询显示结果时可用
  ...
  if (cell_num > 0) {
    char newline = '\n';
    rc = writer_->writen(&newline, 1);//换行符写入
    if (OB_FAIL(rc)) {
      LOG_WARN("failed to send data to client. err=%s", strerror(errno));
      sql_result->close();
      return rc;
  ...
  while (RC::SUCCESS == (rc = sql_result->next_tuple(tuple))) {
    ...
    for (int i = 0; i < cell_num; i++) {
      ...
      if(value.attr_type()==AttrType::DATES){//日期正常展示
          cell_str=cell_str.insert(4,"-");
          cell_str=cell_str.insert(7,"-");
      }
  ...
  if (cell_num == 0) {
    ...
    rc = writer_->writen(send_message_delimiter_.data(), send_message_delimiter_.size());//写
    if (OB_FAIL(rc)) {
