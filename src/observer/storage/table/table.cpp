                 ...
                 const AttrInfoSqlNode attributes[])
{
  ...
  // 判断表文件是否已经存在
  int fd = ::open(path, O_WRONLY | O_CREAT | O_EXCL | O_CLOEXEC, 0600);//判断是否已经创建这个表
  if (fd < 0) {
    if (EEXIST == errno) {
  ...
  // 创建文件
  if ((rc = table_meta_.init(table_id, name, attribute_count, attributes)) != RC::SUCCESS) {//元数据初始化元数据table，表名，元数据字段，table_id等
  ...
  // 记录元数据到文件中
  table_meta_.serialize(fs);//将元数据表序列化到文件流中
  fs.close();

  std::string data_file = table_data_file(base_dir, name);
  BufferPoolManager &bpm = BufferPoolManager::instance();//获取全局唯一的bfferpoolmanger的实例
  ...
  rc = init_record_handler(base_dir);//创建record_handler
  if (rc != RC::SUCCESS) {
...
RC Table::drop(){
  data_buffer_pool_->close_file();//已经做了bufferpoolmanger的相关数据结构的释放
  data_buffer_pool_ = nullptr;
  delete record_handler_;
  record_handler_ = nullptr;
  return RC::SUCCESS;
...
{
  RC rc = RC::SUCCESS;
  rc = record_handler_->insert_record(record.data(), table_meta_.record_size(), &record.rid());//将记录写入对应的rid的磁盘页
  if (rc != RC::SUCCESS) {
...
{
  // 检查字段类型是否一致
  if (value_num + table_meta_.sys_field_num() != table_meta_.field_num()) {//检查插入的字段数量加上系统的和table是否一样
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (field->type() != value.attr_type()) {//检测类型是否一致，不一致则无法将值插入table中
  ...
  char *record_data = (char *)malloc(record_size);//开辟所有字段大小的空间
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    memcpy(record_data + field->offset(), value.data(), copy_len);//将一行所有值拷贝到record_data
  }
...
RC Table::init_record_handler(const char *base_dir)
{
  ...
  rc = record_handler_->init(data_buffer_pool_);//根据bufferpool产生对应的recordmanger
  if (rc != RC::SUCCESS) {
