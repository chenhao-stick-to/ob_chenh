...
RC RecordPageHandler::insert_record(const char *data, RID *rid)
{
  ...
  char *record_data = get_record_data(index);//页面的插入记录的开始地址获取
  memcpy(record_data, data, page_header_->record_real_size);

  frame_->mark_dirty();//写入未提交，标记脏页
...
RC RecordFileHandler::insert_record(const char *data, int record_size, RID *rid)
{
  ...
  // 找到没有填满的页面
  while (!free_pages_.empty()) {//free_pages是没有填充满的页面的集合，找到一个
    current_page_num = *free_pages_.begin();//从头开始遍历
    ...
    record_page_handler.cleanup();
    free_pages_.erase(free_pages_.begin());//从下一个找
  }
  ...
  if (!page_found) {
    Frame *frame = nullptr;
    if ((ret = disk_buffer_pool_->allocate_page(&frame)) != RC::SUCCESS) {//buffer_pool分配一个页面
