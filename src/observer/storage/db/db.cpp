...
RC Db::drop_table(Table* drop_table,const char * table_name)
{
  RC rc = RC::SUCCESS;
  string table_name_tmp=table_name; 
  //获取数据库文件路径
  //std::string table_file_path = table_meta_file(path_.c_str(), table_name);
  std::string targetPath1 = table_name_tmp+".table";
  std::string targetPath2 = table_name_tmp+".data";
  std::string targetPath3 = table_name_tmp+"-index_id.index";
  vector<std::filesystem::path> target_path{targetPath1,targetPath2,targetPath3};
  std::filesystem::path targetPath_root = std::filesystem::current_path() / path_;

  // 遍历目标目录
  for (int i=0;i<3;++i){
  for (const auto& entry : std::filesystem::directory_iterator(targetPath_root)) {
      if (entry.is_regular_file() && entry.path().filename() == target_path[i]) {
          // 删除匹配的文件
  ...
  size_t removedElements = opened_tables_.erase(table_name);
  if(removedElements!=1){
    table_flag=false;
  }
  if(!drop_flag|!table_flag){
